...
/*
 * The blitter, the sprite-map types and the font used to live here. They are
 * needed by every cabinet, so they now live in `../shared/pixel.ts` and are
 * re-exported from this file — Joust's own imports are unchanged, and there is
 * exactly one copy of the glyph table in the tree.
 */
import type { Ink, SpriteMap } from "../shared/pixel";
export type { Ink, SpriteMap };
export {
  blit,
  text,
  textCentered,
